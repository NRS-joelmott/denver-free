...
const GENRES = ['All Genres', 'Rock', 'Jazz', 'Country', 'Classical', 'Hip-Hop'];
...
export default function ConcertsTab() {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 && (
        <div className="empty"><div className="empty-icon">🎵</div>No concerts match those filters</div>
      )}
      {filtered.map(c => (
        <Card
          key={c.id}
          ...
          tags={[
            ...
            ...c.tags.map(t => ({ label: t, color: urgencyColor[t] || 'tag-blue' })),
            { label: c.genre, color: 'tag-blue' },
          ]}
          right={
            <>
              <span className="price-free">FREE</span>
              <span className="dist-text">{c.dist}</span>
            </>
          }
        />
      ))}

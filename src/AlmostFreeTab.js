...
import { almostFree } from './data/index';
...
export default function AlmostFreeTab() {
  ...
  const filtered = almostFree.filter(a => cat === 'All Deals' || a.category === cat);
  ...
  return (
    <div>
      ...
      <div className="ai-bar">
        <input className="ai-input" placeholder="Ask AI: 'best deal for date night?'" value={aiQuery} onChange={e => setAiQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && askAI()} />
        <button className="ai-btn" onClick={askAI} disabled={aiLoading}>{aiLoading ? '...' : '✦ Ask'}</button>
      </div>
      {aiLoading && <div className="ai-result"><div className="ai-result-label">AI is thinking</div><div className="loading-dots"><span /><span /><span /></div></div>}
      {aiResult && !aiLoading && <div className="ai-result"><div className="ai-result-label">✦ AI Recommendation</div>{aiResult}</div>}
      <FilterRow options={CATS} active={cat} onChange={setCat} />
      <SectionHeader title="Near you" count={filtered.length} />
      {filtered.length === 0 && <div className="empty"><div className="empty-icon">💸</div>No deals found</div>}
      {filtered.map(a => (
        <a key={a.id} href={a.url} target="_blank" rel="noopener noreferrer" className="card" style={{ textDecoration: 'none', display: 'block' }}>

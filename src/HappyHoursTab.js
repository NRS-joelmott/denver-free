...
import { happyHours, NEIGHBORHOODS, DAYS } from './data/index';
  ...
  return (
    <div>
      <div className="search-wrap">
        ...
        <input className="search-input" placeholder="Search bar, neighborhood or deal..." value={query} onChange={e => setQuery(e.target.value)} />
      </div>
      <FilterRow options={NEIGHBORHOODS} active={hood} onChange={setHood} />
      <SectionHeader title="Happy hours by day" count={filtered.length} />
      {filtered.length === 0 && <div className="empty"><div className="empty-icon">🍹</div>No happy hours found</div>}
      {Object.entries(byDay).map(([day, items]) => (
        <div key={day} className="day-group">
          <div className="day-label">{day}</div>
          {items.map(h => (
            <a key={h.id} href={h.url} target="_blank" rel="noopener noreferrer" className="card" style={{ textDecoration: 'none', display: 'block' }}>
              <div className="card-row">
                ...
                <div className="hh-body">
                  ...
                  <div className="hh-deals">
                    {h.deals.map((d, i) => <span key={i} className="hh-pill">{d}</span>)}
                  </div>
                </div>
              </div>
            </a>
